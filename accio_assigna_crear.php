...
if ($Fidabsencia==0){
    ...
    //Si té assignació anterior feta cal actualitzar
    $contar = $db->count('Guardia', array('IDPROFESSOR'=>null,'DATA' => $Fdata,'HORA'=>$Fhora,'ACTIVITAT'=>null));				
    if ($contar==0){
      //Si és Convivència cal fer INSERT sense IDPROFESSOR
      $db->insert("Guardia", array('COBERTAPER' => $FselectProfe, 'DATA' => $Fdata, 'HORA' => $Fhora,'ACTIVITAT'=>null));		
    }else{
	  	//Com ja existeix sols cal fer UPDATE
	  $db->update("Guardia", array('COBERTAPER' => $FselectProfe),array('IDPROFESSOR'=>null,'DATA' => $Fdata,'HORA'=>$Fhora,'ACTIVITAT'=>null));		  
    } 
  }
if ($Fidabsencia>0){  
  //GESTIONEM ABSENCIA
