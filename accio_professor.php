...
ob_end_clean();
$qrimage='<img src="data:image/png;base64,'.$qr.'">'
//-----	
?>
<div class="container-fluid">     
	...
	<h5>
	<?php
	//GENERAR RESUM LECTIVES COMPLEMENTARIES
	$QResum = $db->query('SELECT H.TIPUS, COUNT(DISTINCT H.DATA || " " || H.HORA) AS TOTAL
		FROM Horari H WHERE H.IDPROFESSOR = '.$profe_id.' GROUP BY H.TIPUS;'						
	);			
	//echo $db->last();	
	$total_hores=0;
	echo "(";
	foreach($QResum as $resum){
		$resum_tipus = $resum['TIPUS'];
		$resum_count = $resum['TOTAL'];
		echo "".$resum_tipus."=".$resum_count." ";
		$total_hores += $resum_count;
	}
	echo ") ".PROFESSOR_TOTAL.": ".$total_hores;
	?>
	</h5>	
	...
	<div><center><?=$qrimage."<br><b>".$profe_nomprofe."</b><br>".$data;?></center></div>
 </div>