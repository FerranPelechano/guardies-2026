...
$query = "SELECT P.* FROM Professor AS P LEFT JOIN Substitucions AS S ON S.SUBSTITUT=P.ID WHERE S.PROFE IS NOT NULL AND S.A = (SELECT MAX(S2.A)FROM Substitucions S2 WHERE S2.SUBSTITUT = P.ID) AND S.A<'".date("Y-m-d")."'";
//echo $query."<br>";
...
$query ="SELECT PRO.*, (SELECT H.DATA FROM Horari AS H WHERE (TIPUS='AF' OR TIPUS='AFD') AND H.IDPROFESSOR=PRO.ID) AS DATA,(SELECT H.HORA FROM Horari AS H WHERE (TIPUS='AF' OR TIPUS='AFD') AND H.IDPROFESSOR=PRO.ID) AS HORA FROM Professor AS PRO ORDER BY PRO.NOM";
