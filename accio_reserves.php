            ...
            foreach ($config_intervals as $hora){		
				...
				foreach ($columna as $element){
					...
					*/
					...
					//Mire en quins dies del calendari estem per mostrar horaris o no (Inici Curs - Final de Curs)
					if ($element[0] >= $calendari_inicial && $element[0] <= $calendari_final) {						    					
						$horaris = $db->select(
	    			    	'Horari', 
	    			    	array('Horari.ID','Horari.AULA','Horari.GRUP','Horari.MATERIA'),
	    					array('AND' => array('DATA' => ''.$dia.'', 'HORA'=>''.$hora.'','AULA' =>''.$Faula.''), 'ORDER' => 'AULA')
						...
						$horaris = []; // No mostrar horaris fora del calendari (Inici Curs - Final de Curs) per poder reservar
					}
